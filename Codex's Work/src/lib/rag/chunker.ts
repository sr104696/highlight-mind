...
export function slidingChunks(
  ...
  while (i < stream.length) {
    ...
    // Sentence-aware: extend to next sentence end within +20 tokens, or back off to previous within window
    ...
    // Determine page assignment: chunks straddling pages get tagged to the LAST page they enter
    ...
    // bbox computed only from tokens on the chunk's assigned page
    ...
    out.push({
      ...
      bbox,
      lines: lines.filter((_, idx) => {
        // keep all lines but they share page assignments via bbox
        return true;
      }),
    });
